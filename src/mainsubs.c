...
void show_options ( struct options_st *opts , int v ) {
    ...
    fprintf(stderr, "\t %s=[%s] rplib_mode[%d] seed=[%ld]\n", "L:Fname", opts->rplib_fname,opts->rplib_mode,opts->rp_seed);
...
int get_options (int argc, char *argv[], struct options_st *opts) {
    ...
    opts->rp_seed = seed ;             /* No effect unless the program is run in Library mode */
...
/* options added by JGM: -C, -D,  -M, -O, -P, -R, -T, -N, -E, -S, -W, -X, -U and -0 thru -9  */
     ...
     case 'L':
         ...
         opts->rplib_mode = 1 ;
         opts->rp_seed = (opts->seed_provided > 0 ) ? opts->seed : 0 ;  /* Note 0 is a valid seed in RP_lib mode */
