...
typedef struct side_fitstat_st {
   ...
   int fitids[4];				/* sorted so that fitids[i] is the number of the suit whose length is i fitlen[i] */
} SIDE_FIT_k ;

typedef struct trump_fit_st {
   int dummy ;    			/* The compass North(0), East(1), South(2), West(3) for Dummy */
   ...
   int fit_len  ;  			/* Longest fit; if = 7 promises a 5-2 Not 4-3 or 6-1 If no 7+ fit should be -1 */
