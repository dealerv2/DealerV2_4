...
struct bucket_st {
   ...
   char Dir   ;         	/* put at end so not to mess up alignment */
} ;
struct acubf_st {
...
struct options_st {
  ...
  char 		log_fname[128];			// -l   Filename for saving deals in Deal52 fmt. -l[wN:]<fname>; N suppresses calculating dd solutions */
  char 		log_fmode[8];				
  int  		log_dds ;
  int	      log_wanted ;           // main loop will write LOG52 file if set; unless flog is NULL 
