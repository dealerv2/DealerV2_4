...
/* 2024-01-28 -- With Roth now at 14 Metrics; 15 if JGM mods to Roth */
/* Boosted to 128 results allows 7 results for up to 16 metrics; 6 res for NTx(side,Hand1,Hand2) and suitx(side, H1, H2) */
typedef struct user_eval_results_st { // this struct holds the results for one side. the usual case for this kind of query
   union res_u_ut {
      ...
      struct detailed_res_st {  // suggested layout for convenience. will correspond to syntax in dealer input file
         int side_tot[48] ;        //0  - 47  48 metrics for side as a whole. Usual case otherwise dealer is adequate.
         int side_by_suit[4][8] ;  //48 - 79  32 diff metrics for the side, broken down by suit.[CDHS][wxyz]
         int hand_tot[2][8];       //80 - 95  16 hand results 8 diff metrics total for each hand
         int hand_suit[2][4][4];   //96 - 127 32 results by suit; 4 diff metrics for each hand-suit combo.
      } dr ;                       // detailed result
