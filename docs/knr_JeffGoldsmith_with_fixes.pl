...
# Usage:
#        Reads from STDIN.
#        Suits should be in order Spades Hearts Diamonds Clubs separated by SPACES (or commas)
...
while (<STDIN>)    {
    ...
    if ($cards != 13)   {
      print "Sorry, this 'hand' has $cards cards.  Please try again.\n";
       ...
       $val     = &cccc(@_);
       $dkval   = &dkcccc(@_);
       # $hcp4321 = &stdhcp(@_); stdhcp not in this one. in KnR.pl with JGM additions.
...
JGM Comments:
1. Assuming the LR shows 4 card support, there is a 10 fit, so the 2 points for the stiff club become 4 (no pts for 1st dblton) and the hand
is now worth 14.95. I don't see 16 unless the 1st doubleton was counted (in error?) as 1 and is now worth 2 so a total of 15.95 pts. 
Regardless facing a LR (8 losers) a hand with 6.5 losers would usually bid game. 
